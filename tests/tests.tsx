...
['Student_2022_001', 'Student_2022_003']`;
  ...
  render(
    <DataInput 
      ...
      setClassrooms={jest.fn()}
      students={[]} 
      setStudents={setCourses as any} // gerekirse dummy
    />
  );

  // 1) Sadece butonu hedefle (rol = button)
  const addButton = screen.getByRole('button', { name: 'dataInput.addNew' });
  fireEvent.click(addButton);

  // 2) Modal açıldı mı? Bunu buton text’iyle değil,
  //    modal içindeki form label’ıyla test edelim
  await waitFor(() => {
    expect(
      screen.getByLabelText('dataInput.courseCode')
    ).toBeInTheDocument();
  });
