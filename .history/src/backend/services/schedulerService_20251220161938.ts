...
function solveGreedy(
    ...
    for (let i = 0; i < courses.length; i++) {
        ...
        for (const timeSlot of timeSlots) {
            ...
            // 1. ADIM: Bu saat dilimi için öğrenci çakışması (Student Conflict) var mı?
            // Herhangi bir odada bu dersi yapabilir miyiz diye genel bir kontrol yapalım.
            // Not: Pratik olması için ilk müsait odayla 'isSafe' kontrolü yapıyoruz, çünkü isSafe içindeki
            // öğrenci kontrolü odadan bağımsızdır.
            if (!isSafe(course, classrooms[0], timeSlot, context)) {
                // Eğer öğrencilerin bu saatte başka sınavı varsa, odaya bakmaya gerek yok.
                continue; 
            }

            // 2. ADIM: Bu saatteki BOŞ odaları bul ve Kapasiteye göre (Büyük -> Küçük) sırala
            ...
            const availableRooms = classrooms
                ...
                .sort((a, b) => b.capacity - a.capacity); // En büyük odayı önce dene

            // 3. ADIM: Öğrencileri odalara dağıt (SPLIT LOGIC)
            ...
            for (const room of availableRooms) {
                ...
                // Odayı listeye ekle ve kapasitesini düş
                roomsToUse.push(room);
                studentsRemaining -= room.capacity;
            }

            // 4. ADIM: Eğer kalan öğrenci yoksa (yani odalar yettiyse), atamayı yap
        ...
        if (!placed) {
            ...
            console.log(`FAILED at course ${course.code}: Not enough room capacity or time slots.`);
