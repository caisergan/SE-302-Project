...
function testFR8(): boolean {
  ...
  const behaviorOK = csv.split('\n').length === 2 && csv.includes('e1,CS101,R101');
  logLine(behaviorOK, 'Behavior: schedule can be serialized to a simple CSV format.');

  // Integration: ScheduleView’te export butonu var mı?
  const svCode = readSource('src/components/ScheduleView.tsx');
  const hasExportButton = svCode.includes("t('schedule.export'");

  // (Şu an için gerçek file download/FS yok, sadece UI)
  logLine(hasExportButton, 'Integration: ScheduleView contains an Export button (label schedule.export).');

  const integrationOK = hasExportButton;
  const passed = behaviorOK && integrationOK; // burada fail olabilir ama önemli olan test etmesi :)
...
function testFR11(): boolean {
  ...
  for (let i = 0; i < aliceExams.length - 1; i++) {
    const gap = aliceExams[i + 1].startTime.getTime() - aliceExams[i].endTime.getTime();
    if (gap <= 0) {
      hasGap = false;
      break;
    }
  ...
  logLine(hasGap, 'Behavior: student exams have a positive gap (no back-to-back sessions).');
  const behaviorOK = hasGap;

  // Integration: App’te handleFinalizeSchedule içinde öğrenci bazlı kontrol var mı?
  const appCode = readSource('src/app.tsx') || '';
  const referencesStudentsInSchedule =
    appCode.includes('students') &&
    appCode.includes('courses') && appCode.includes('ExamSession');

  // şu an projede student-based constraint implement edilmemiş, bu yüzden FAIL olması normal
  logLine(
    referencesStudentsInSchedule,
    'Integration: handleFinalizeSchedule references students for constraints (currently expected FALSE).'
  );

  const integrationOK = referencesStudentsInSchedule; // büyük ihtimal false
  const passed = behaviorOK && integrationOK;
  console.log(`\nFR11 => ${passed ? 'PASS ✅' : 'FAIL ❌'} (muhtemelen şu an FAIL, constraint eksik)\n`);
  return passed;
}
